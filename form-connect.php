...
$response = file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=".$RCsecretkey."&response=".$_POST['g-recaptcha-response']);
...
if ($responsekeys->success) {
  if ($_POST['username'] == "") {
       ...
       )
    {
      ...
      if ($_POST[md5('address'.$_POST['ip'].$salt.$_POST['timestamp'])] != "")
      $Message .= $_POST[md5('address'.$_POST['ip'].$salt.$_POST['timestamp'])]."\n";

      if ($_POST[md5('citystatezip'.$_POST['ip'].$salt.$_POST['timestamp'])] != "")
      $Message .= $_POST[md5('citystatezip'.$_POST['ip'].$salt.$_POST['timestamp'])]."\n";

      if ($_POST[md5('message'.$_POST['ip'].$salt.$_POST['timestamp'])] != "")
